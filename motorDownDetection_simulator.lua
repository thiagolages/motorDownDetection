...
-- times
local WAIT_TIME_MODE_AUTO_MS        = 1000 -- wait 1s after AUTO is enabled before starting to check for problems
local TIME_DETECTION_THRESHOLD_MS   = 3000 -- (3s) Time frame to run this motor down detection algorithm (milliseconds)
local WAIT_TIME_MOTOR_DOWN_MS       = 1000 -- (300ms) Time to wait before considering a motor failure, in milliseconds
...
local function checkMotorDown() -- main loop
    ...
    if (arming:is_armed()) then
        ...
        if (mode == COPTER_MODE_GUIDED) then
            ...
            startTimeModeAuto = -1 -- reset AUTO mode counter
        end
    else -- if not Armed
